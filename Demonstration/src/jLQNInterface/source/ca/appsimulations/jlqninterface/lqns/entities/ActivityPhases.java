...
public class ActivityPhases extends ActivityPhasesType {
	...
	public ActivityPhases(Model workspace) {
		this(workspace, "", null, 1);
	}

	public ActivityPhases(Model workspace, String name, Entry entry, int phase) {
		super(workspace, name, 0.0f, phase, entry);

		workspace.addActivity(this);
	...
	public ActivityPhases duplicateDepth(Entry parentEntry) {
		...
		ActivityPhases ap = new ActivityPhases(this.workspace, newName, parentEntry, this.phase);
		ap.isDuplicate = true;
	...
	public void linkSynchCalls() {
		for (SynchCall s : synchCalls) {
			Entry dest = workspace.getEntryByName(s.getStrDestEntry());
			s.setDestEntry(dest);
		}
	}
