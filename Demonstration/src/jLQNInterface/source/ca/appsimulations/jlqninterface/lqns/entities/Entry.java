...
public class Entry extends EntryType {
	...
	public Entry(Model workspace) {
		this(workspace, "", null);
	}

	public Entry(Model workspace, String name, Task task) {
		super(workspace, name, EntryAcType.PH1PH2);
		this.id = nextId++;
		this.task = task;
	...
	public Entry duplicateDepth(Task parentTask) {
		...
		Entry e = new Entry(this.workspace, newName, parentTask);
		e.isDuplicate = true;
		...
		ArrayList<Entry> srcEntries = workspace.getSrcEntries(this.name);
