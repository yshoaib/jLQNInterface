...
public class BottleneckAlgorithm extends Algorithm {
	...
	@Autowired
	public BottleneckAlgorithm(LqnModel lqnModel) {
		super(lqnModel);
	...
	public void setup() {
		...
		log.debug("Parsing input file " + inputFilePath);
		try {
	...
	private boolean findBottlenecks(ArrayList<Entity> bStrengthTable, ArrayList<Entity> unchangeable, boolean solveModel) {
		...
		bStrengthTable.addAll(bottleneckIdentifier.generateBStrengthTable(satThreshold, unchangeable, this.bottleneckMaxBStrengthTaskOnly));
	...
	private boolean solveAndParseModel() {
		...
		log.debug("output model to: " + outputFilePath);
		log.debug("");
	...
	public float findMaxResourcePerformance(ArrayList<Entity> bSet) {
		...
		while (!flag && (tmpMaxProcsPerVM >= 1 || tmpMaxVMReplicas >= 1)) {
			log.info("-----------------------------------------------------------------------------");
			log.info("Solving to find maxResourcePerformance: maxVMReplicas: " + tmpMaxVMReplicas + " maxProcsPerVM: " + tmpMaxProcsPerVM);
			log.info("-----------------------------------------------------------------------------");
	...
	private boolean processProcessorMultiplication(Processor p, int multiplicityAdded, ArrayList<Entity> unchangeable) {
		if (p.getMultiplicity() >= this.maxProcsPerVM) {
			...
			log.info(p.getName() + " " + "multiplicity: " + p.getMultiplicity() + " has reached maxReplicas: " + maxProcsPerVM);
			return false;
		}
    ...
    public void run() {
		...
		if (maxResResponse == LqnConstants.UNDEF.getConstantValue()) {
			...
			log.info("ResponseTime Objective: " + responseTimeObjective);
		...
		while (true) {
			...
			if (this.responseTimeObjective >= responseTime) {
				...
				log.info(String.format("Objective: %.3f, ResponseTime: %.3f", responseTimeObjective, responseTime));
				break;
			}
