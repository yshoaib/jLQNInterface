...
public class Task extends TaskType {
	...
	public Task(Model workspace) {
		this(workspace, "", null);
	}

	public Task(Model workspace, String name, Processor processor) {
		super(workspace, name, false);
		this.id = nextId++;
	...
	public String getBelowQutotationStrArray() {
		return Utility.listToQutotationStrArray(below);
	}
	...
	private Task duplicateDepth(Processor parentProc) {
		...
		Task t = new Task(dupParent.workspace, newName, parentProc);
		t.isDuplicate = true;
	...
	public boolean setupDuplication(int duplicationCount, boolean duplicateParentProc, Processor parentProc) {
		...
		for (Entry e : entries) {
			ArrayList<Entry> srcEntries = workspace.getSrcEntries(e.getName());
